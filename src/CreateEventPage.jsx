  ...
  useEffect(() => {
    ...
    const loadPlace = async () => {
      ...
      try {
        ...
        if (!isMounted) return;
        setPlaceName(place?.name || place?.nombre || '');
      } catch (err) {
        const message = err?.message || 'Error cargando el sitio';
      ...
      <section className="relative pt-24 pb-16 px-4">
        <div className="max-w-4xl mx-auto">
          <div className="mb-8">
            ...
            <h1 className="text-4xl font-bold text-slate-900 mb-2">Crear evento</h1>
            <p className="text-slate-600">
              {placeName ? `Agrega un evento al sitio ${placeName}` : 'Agrega un nuevo evento'}
            </p>
          </div>
